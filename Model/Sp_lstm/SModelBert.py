...
class SModelBert(nn.Module):
    def __init__(self,bert_size=3072,in_size=1024, g_size=600, linear_h_size=1024, out_size=1, dropout=0.1):
        super(SModelBert, self).__init__()
        self.in_size = in_size
        self.g_size = g_size
        self.out_size = out_size
        self.linear_h_size = linear_h_size
        self.li1=nn.Linear(bert_size,in_size)
        ...
        self.li = nn.Linear(in_size*2,in_size*2)
        self.S_LSTM2 = S_LSTM(x_size=in_size*2, g_size=g_size, out_size=in_size*2)
        self.dropout = nn.Dropout(dropout)
        self.outlinear1 = nn.Linear(in_size * 4, linear_h_size*2)
        self.outlinear2 = nn.Linear(linear_h_size*2, out_size)
