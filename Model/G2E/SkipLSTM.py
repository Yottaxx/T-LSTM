...
class SkipLSTM(nn.Module):
    def __init__(self, vocab_len, in_size=300, lstm_h_size=1200, g_out_size=300, at_size=1200, l_h_size=300, out_size=1,
        ...
        self.linea1 = nn.Linear(in_size, in_size * 2)
        self.linea2 = nn.Linear(in_size, in_size * 2)
        # self.pregcn = GraphConvolution(in_size,in_size)
        self.lstm = nn.LSTM(in_size, int(in_size / 2), num_layers=1, batch_first=True, bidirectional=True)
        self.lstm2 = nn.LSTM(in_size, int(in_size/2), num_layers=1, batch_first=True, bidirectional=True)
        self.lstm3 =nn.LSTM(in_size, in_size, num_layers=1, batch_first=True, bidirectional=True)
        ...
        self.p1 = nn.Parameter(torch.tensor([0.2]))
        self.p2 = nn.Parameter(torch.tensor([0.8]))
    ...
    def forward(self, x, adj, trigger, mask=None):
        ...
        x1, _ = self.lstm(x)
        x2, _ = self.lstm2(x1)
        x = self.p1 * (F.relu(self.linea1(x1))) + self.p2 * (F.relu(self.linea2(x2)))
