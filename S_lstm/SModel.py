...
class SModel(nn.Module):
    def __init__(self, vocab_len,emb_size=300, in_size=300, g_size=200, linear_h_size=300, out_size=1, dropout=0.3):
        super(SModel, self).__init__()
        self.in_size = in_size
        self.g_size = g_size
        self.out_size = out_size
        self.linear_h_size = linear_h_size
        self.embedding = nn.Embedding(vocab_len, emb_size,padding_idx=1)
        self.le2s=nn.Linear(emb_size,in_size)
        self.S_LSTM = S_LSTM(x_size=in_size, g_size=g_size, out_size=in_size)
        self.li = nn.Linear(in_size*2,in_size*1)
        self.S_LSTM2 = S_LSTM(x_size=in_size*1, g_size=g_size, out_size=in_size*1)
        self.dropout = nn.Dropout(dropout)
        self.outlinear1 = nn.Linear(in_size * 2, in_size*1)
        self.outlinear2 = nn.Linear(in_size*1, out_size)
